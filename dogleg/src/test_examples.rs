...
mod problems;
use argmin_dogleg::ArgminWrapper;
use problems::*;
...
fn argmin_test_rosenbrock() {
    ...
    let initial = initial.map(|val| 10. * val);
    let result = argmin_dogleg::run_dogleg(problem, initial).unwrap();

    panic!("rosenbrock argmin: {:?}", result);
}
