...
impl LinearFullRank {
    pub fn new(params: OVector<f64, U5>, m: usize) -> Self {
        Self { params, m }
    }
}
...
where
    DefaultAllocator: Allocator<P>,
{
    pub fn new(params: OVector<f64, P>, _n: usize) -> Self {
        Self { params }
    }
}
...
fn derivative<F: num_traits::Float + RealField + Copy>(
    x: F,
    f: impl Fn(F) -> Option<F>,
) -> Option<F> {
    const STEPS: usize = 15;
    let step_ratio: F = convert(STEP_RATIO);
    let mut quotients = Vec::with_capacity(STEPS);
    let mut h = num_traits::Float::max(
        num_traits::Float::ln(F::one() + num_traits::Float::abs(x)),
        F::one(),
    );
    let two: F = convert(2.);
    for _ in 0..STEPS {
...
fn extrapolate<F: RealField + num_traits::Float>(evaluations: Vec<F>) -> Option<F> {
...
fn outlier_aware_minimum<F: num_traits::Float>(mut values: Vec<(F, F)>) -> Option<F> {
    let num = values.len();
    let mut not_nan = num;
    // move NaN to the end
    for i in 0..num {
...
fn wynn_extrapolate<F: num_traits::Float>(estimates: Vec<F>) -> Option<Vec<(F, F)>> {
    let num = estimates.len();
